...
create table uni_inbox.email_accounts (
...
create index email_accounts_due_idx on uni_inbox.email_accounts (status, next_sync_at);

alter table uni_inbox.email_accounts enable row level security;
create policy "owner read accounts" on uni_inbox.email_accounts
...
revoke select on table uni_inbox.email_accounts from authenticated, anon;
grant select (id, owner_id, label, email_address, color, provider_preset,
              status, last_error, created_at)
  on table uni_inbox.email_accounts to authenticated;

-- Per-account IMAP sync cursor. Service-role only: RLS on, zero policies.
create table uni_inbox.sync_state (
  account_id        uuid primary key references uni_inbox.email_accounts(id) on delete cascade,
...
alter table uni_inbox.sync_state enable row level security;
